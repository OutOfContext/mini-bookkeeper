...
const Employees: React.FC = () => {
  ...
  const fetchTodayShifts = async () => {
    try {
      ...
      if (activeSession) {
        // Get shifts only for the current session
        const sessionStartTime = new Date(activeSession.startTime)
        const now = new Date()
        
        const response = await apiService.getShiftsByRange(
          sessionStartTime.toISOString(),
          now.toISOString()
        )
        setTodayShifts(response.data)
      } else {
        // No active session - clear shifts
        setTodayShifts([])
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="card text-center">
          <h3 className="text-lg font-semibold text-gray-900">Session-Löhne</h3>
          <p className="text-2xl font-bold text-warning-600">
            €{getTotalWagesToday().toFixed(2)}
          </p>
          <p className="text-sm text-gray-500 mt-1">Nur aktuelle Session</p>
        </div>
      </div>
