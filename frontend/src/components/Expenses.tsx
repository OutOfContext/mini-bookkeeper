...
const Expenses: React.FC = () => {
  ...
  const fetchExpenses = async () => {
    try {
      ...
      if (activeSession) {
        // Get expenses only for the current session
        const sessionStartTime = new Date(activeSession.startTime)
        const now = new Date()
        
        const expensesRes = await apiService.getExpensesByRange(
        ...
        const sessionTotal = expensesRes.data.reduce((sum: number, expense: any) => sum + expense.amount, 0)
        setTodayTotal(sessionTotal)
      } else {
        // No active session - clear expenses
        setExpenses([])
        setTodayTotal(0)
      }
    } catch (error) {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="text-lg font-semibold text-danger-600">
          Session-Gesamt: €{todayTotal.toFixed(2)}
        </div>
      </div>
