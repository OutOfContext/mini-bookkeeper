...
// Get shifts by date range for session-specific calculations
router.get('/shifts/range', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    console.error('Get shifts by range error:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});
